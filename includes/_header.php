<header class="Header py-md-0 js-details-container Details flex-wrap flex-md-nowrap px-3" role="banner"
        style="background-color:<?php echo($this->params->get('topMenuBGColor') != "" ? $this->params->get('topMenuBGColor') : '#103157') ?> !important;">
    ...
    <div class="Header-item Header-item--full flex-justify-center d-md-none position-relative">
		...
		if ($user->id != 0): ?>
            <a class="Header-link" href="<?php echo $dashboardlink; ?>"><img class="avatar avatar-user"
                                                                             src="<?php echo $avatar_image; ?>"
                                                                             width="20" height="20"
                                                                             alt="<?php echo $user->username; ?>"/> <?php $config = JFactory::getConfig();
				echo $config->get('sitename'); ?></a>
		<?php else: ?>
            <a class="Header-link" href="<?php echo $dashboardlink; ?>"><?php $config = JFactory::getConfig();
				echo $config->get('sitename'); ?></a>
		<?php endif; ?>
    </div>
